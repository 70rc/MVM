...
def read_meta_from_spreadsheet (df, filename) :
  ...
  for idx in range(len(df))  :
    ...
    meta[ key ] = {
      ...
      'SimulatorDirName': ( df["simulator_dirname"].iloc[idx] if "simulator_dirname" in df else "." ) ,
      'SimulatorFileName': df["simulator_filename"].iloc[idx] ,
      'Campaign': df["campaign"].iloc[idx] ,
      'MVM_filename' : df["MVM_filename"].iloc[idx],
      'MVM_mapping' : ( df["MVM_mapping"].iloc[idx] if "MVM_mapping" in df else None ) ,
      'MVM_dirname' : ( df["MVM_dirname"].iloc[idx] if "MVM_dirname" in df else None ) ,
